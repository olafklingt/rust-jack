...
#[test]
fn client_can_open() {
    test_tools::open_test_client("client_can_open");
}
...
fn client_fails_to_open_with_large_name() {
    let name = (0..=*CLIENT_NAME_SIZE)
        ...
        .collect::<Vec<&str>>()
        .join("_");
    let r: Result<(Client, ClientStatus), Error> =
        Client::new(&name, ClientOptions::NO_START_SERVER);
    let r = r.unwrap();
    assert_ne!(r.0.name(), name);
    assert_eq!(r.1, ClientStatus::NAME_NOT_UNIQUE);
}

#[test]
fn client_can_be_named() {
    let name = "client_can_be_named";
    let (c, _) = test_tools::open_test_client(name);
    assert_eq!(c.name(), name);
}

#[test]
fn client_can_activate() {
    let (c, _) = test_tools::open_test_client("client_can_activate");
    let _ac = c.activate_async((), ()).unwrap();
}

#[test]
fn client_can_set_buffer_size() {
    let (c, _) = test_tools::open_test_client("client_can_set_buffer_size");
    let ac = c.activate_async((), ()).unwrap();
    let c = ac.as_client();
...
#[test]
#[ignore = "doesn't fail with pipewire"]
fn client_detects_bad_buffer_size() {
    let (c, _) = test_tools::open_test_client("client_detects_bad_buffer_size");
    let initial_size = c.buffer_size();
...
fn client_can_deactivate() {
    let (c, _) = test_tools::open_test_client("client_can_deactivate");
...
fn client_knows_buffer_size() {
    let (c, _) = test_tools::open_test_client("client_knows_buffer_size");
...
fn client_knows_sample_rate() {
    let (c, _) = test_tools::open_test_client("client_knows_sample_rate");
...
fn client_knows_cpu_load() {
    let (c, _) = test_tools::open_test_client("client_knows_cpu_load");
    let _load = c.cpu_load();
}

#[test]
fn client_can_estimate_frame_times() {
    let (c, _) = test_tools::open_test_client("client_knows_frame_times");
    let ac = c.activate_async((), ()).unwrap();
    let c = ac.as_client();
...
fn client_debug_printing() {
    let (c, _) = test_tools::open_test_client("client_has_debug_string");
    let got = format!("{c:?}");
    assert_ne!("", got);
}
...
fn client_can_use_ringbuffer() {
    let (c, _) = test_tools::open_test_client("client_can_use_ringbuffer");
...
fn client_uuid() {
    let (c1, _) = test_tools::open_test_client("uuidtest-client1");
    let (c2, _) = test_tools::open_test_client("uuidtest-client2");
    ...
    let uuid3s = {
        let (c3, _) = test_tools::open_test_client("uuidtest-client3");
        c3.uuid_string()
    };
